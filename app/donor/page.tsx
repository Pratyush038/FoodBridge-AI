...
import { Upload, History, TrendingUp, Users, MapPin, Clock, Bell, Plus, ExternalLink, Phone, Search, Filter, Loader2 } from 'lucide-react';
...
export default function DonorDashboard() {
  ...
  const fetchDonorData = useCallback(() => {
    if (session?.user) {
      ...
      const unsubscribe = getDonationsByDonor(userId, (donationData) => {
        console.log('📦 Donations refreshed:', donationData.length);
        setDonations(donationData);
        
        // Create map markers from donations
        const markers: MapMarker[] = donationData.map(donation => ({
          id: donation.id!,
  ...
  useEffect(() => {
    ...
    try {
      // Fetch donor data
      unsubscribeDonations = fetchDonorData();

      // Listen to active requirements
      unsubscribeRequirements = listenToActiveRequirements((requirements) => {
  ...
  const handleContactOrganization = (requirement: any) => {
    ...
    <div className="min-h-screen bg-gray-50 pt-16">
      ...
      <div className="container mx-auto p-4 md:p-6 lg:p-8">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Manage your food donations and connect with organizations in need</p>
        </div>
